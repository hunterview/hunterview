/**
 * 미블 (mrblog.net) 크롤러
 * - Playwright 사용 (SPA/React 기반)
 * - URL: https://www.mrblog.net/campaigns
 */

const { chromium } = require('playwright');

const BASE_URL = 'https://www.mrblog.net';
const LIST_URL = `${BASE_URL}/campaigns`;

module.exports = async function crawlMrblog() {
  ...
  try {
    ...
    // 캠페인 카드 로딩 대기
    await page.waitForSelector('a[href*="/campaign/"], a[href*="/campaigns/"]', { timeout: 15000 }).catch(() => {});
    ...
    const items = await page.evaluate((baseUrl) => {
      ...
      document.querySelectorAll('a[href*="/campaign"]').forEach(el => {
        ...
        const idMatch = href.match(/\/campaign[s]?\/(\d+|[a-zA-Z0-9\-]+)/);
        if (!idMatch) return;

        const rawId = idMatch[1];
        const id = `mrblog_${rawId}`;
        ...
        const titleEl = el.querySelector('h2, h3, h4, [class*="title"], [class*="name"], [class*="Title"]');
        ...
        let dday = 30;
        const ddayMatch = fullText.match(/D-(\d+)|마감\s*(\d+)일|(\d+)일\s*남/);
        if (ddayMatch) dday = parseInt(ddayMatch[1] || ddayMatch[2] || ddayMatch[3]);
        if (/오늘\s*마감/.test(fullText)) dday = 0;
        if (/마감됨|모집\s*완료/.test(fullText)) dday = -1;
    ...
    throw new Error(`미블 크롤링 실패: ${err.message}`);
  } finally {
    await browser.close();
  }
};
