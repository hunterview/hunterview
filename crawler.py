...
def detect_type(text):
    if "릴스" in text or "숏츠" in text or "숏폼" in text or "클립" in text: return "숏폼"
...
def extract_reward(text):
    m = re.search(r'\[?(\d[\d,]+원\s*상당)\]?', text)
    if m: return m.group(1)
    m = re.search(r'\[?(\d+,?\d*만원\s*상당)\]?', text)
    if m: return m.group(1)
    return ""
            ...
            try:
                ...
                if title and len(title) > 4:
                    results.append({
                        ...
                        "location": location, "review_type": detect_type(text),
...
async def crawl_pavlovu(page):
    ...
    results = []
    try:
        await page.goto("https://pavlovu.com/review_campaign_list.php", timeout=25000)
        await page.wait_for_timeout(3000)
        # 실제 링크 패턴: review_campaign.php?cp_id=숫자
        items = await page.query_selector_all("a[href*='review_campaign.php?cp_id']")
        if not items:
            items = await page.query_selector_all(".tanz_campaign_list_wrap li a")
        for item in items[:50]:
            try:
                href = await item.get_attribute("href") or ""
                if not href: continue
                link = "https://pavlovu.com/" + href.lstrip("/") if not href.startswith("http") else href
                text = await item.inner_text()
                ...
                if title and len(title) > 4:
                    results.append({
                        "title": clean(title), "reward": reward, "platform": "파블로체험단",
...
# ────────────────────────────────────────────
async def crawl_tble(page):
    print("🔍 티블 크롤링...")
    results = []
    try:
        for url in ["https://tble.kr/category.php", "https://tble.kr/category.php?type=p"]:
            await page.goto(url, timeout=25000)
            await page.wait_for_timeout(2000)
            items = await page.query_selector_all("a[href*='view.php?cp_id']")
            for item in items[:40]:
                try:
                    href = await item.get_attribute("href") or ""
                    link = "https://tble.kr/" + href.lstrip("./") if not href.startswith("http") else href
                    text = await item.inner_text()
                    ...
                    img_el = await item.query_selector("img")
                    image = await img_el.get_attribute("src") if img_el else ""
                    if image and not image.startswith("http"): image = "https://tble.kr/" + image.lstrip("./")
                    dday_match = re.search(r'(\d+)일\s*남음', text)
                    deadline = ""
                    if dday_match:
                        d = datetime.now() + timedelta(days=int(dday_match.group(1)))
                        deadline = d.strftime("%Y-%m-%d")
                    if title and len(title) > 4:
                        results.append({
                            "title": clean(title), "reward": reward, "platform": "티블",
                            "link": link, "image": image, "deadline": deadline,
                            "location": detect_location(text), "review_type": detect_type(text),
                            "applied": 0, "total": 10
                        })
...
async def crawl_mrblog(page):
    ...
    results = []
    try:
        await page.goto("https://www.mrblog.net/campaigns", timeout=30000)
        ...
        for item in items[:50]:
            try:
                ...
                if title and len(title) > 4:
                    results.append({
                        "title": clean(title), "reward": reward, "platform": "미블",
                        "link": link, "image": image, "deadline": parse_deadline(text),
                        "location": detect_location(text), "review_type": detect_type(text),
                        "applied": 0, "total": 10
                    })
...
async def crawl_4blog(page):
    ...
    results = []
    try:
        for url in ["https://4blog.net", "https://4blog.net/list"]:
            await page.goto(url, timeout=25000)
            await page.wait_for_timeout(3000)
            ...
            for item in items[:60]:
                try:
                    ...
                    if "4blog.net" not in href and not href.startswith("/"): continue
                    link = "https://4blog.net" + href if href.startswith("/") else href
                    if not re.search(r'/\d+|/view|/campaign|/detail', href): continue
                    text = await item.inner_text()
                    lines = [l.strip() for l in text.split('\n') if l.strip() and len(l.strip()) > 3]
                    if not lines: continue
                    title = max(lines, key=len)
                    ...
                    image = await img_el.get_attribute("src") if img_el else ""
                    results.append({
                        "title": clean(title), "reward": reward, "platform": "포블로그",
                        "link": link, "image": image, "deadline": parse_deadline(text),
                        "location": detect_location(text), "review_type": detect_type(text),
                        "applied": 0, "total": 10
                    })
        ...
        for url, loc in urls:
            ...
            for item in items[:30]:
                try:
                    ...
                    if re.match(r'https://www\.modan\.kr/?$|.*/(?:matzip|beauty|delivery|product)$', link): continue
                    text = await item.inner_text()
                    ...
                    results.append({
                        "title": clean(title), "reward": reward, "platform": "모두의체험단",
...
async def main():
    ...
    async with async_playwright() as pw:
        ...
        all_campaigns += await crawl_tble(page)
