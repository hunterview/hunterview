...
function inferLocation(title, text) {
  // 1. 제목의 [도시/동네] 브래킷에서 직접 추출
  const bracket = parseBracketCity(title);
  ...
  const rules = [
    ...
    [/홍대|마포|합정/,               '서울 마포'],
    [/제주/,                         '제주도'],
    [/수원|성남|용인|안산|안양|화성|고양|파주|의정부|시흥/, '경기'],
    [/경기/,                         '경기'],
    [/인천/,                         '인천'],
    [/대구/,                         '대구'],
    [/대전/,                         '대전'],
...
function parseBracketCity(title) {
  // [서울/강남구] 또는 [서울 강남] 형식에서 도시 추출
  const m = title.match(/^\[([^\]/,]+)/);
  if (!m) return null;
  const city = m[1].trim();
  const cityRules = [
    ...
    [/^제주/,                         '제주도'],
    [/^세종/,                         '세종'],
    [/수원|성남|용인|고양|안산|안양|화성|파주|의정부|시흥/, '경기'],
    [/^경기/,                         '경기'],
    [/천안|아산|충남|충북|청주|공주|논산/, '충청'],
    [/전주|전북|전남|광양|여수|순천/, '전라'],
    [/창원|진주|포항|경남|경북|구미|김해|통영/, '경상'],
    [/강릉|춘천|강원|원주/,           '강원'],
  ];
  for (const [re, loc] of cityRules) if (re.test(city)) return loc;
  return null;
}
